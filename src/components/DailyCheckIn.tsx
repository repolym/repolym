...
export default function DailyCheckIn() {
    ...
    useEffect(() => {
        if (metrics) {
            setBedtime(metrics.bedtime ? metrics.bedtime.slice(0, 5) : '');
            setWakeTime(metrics.wake_time ? metrics.wake_time.slice(0, 5) : '');
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div>
                            <label htmlFor="checkin-bedtime" className="block text-xs text-slate-400 mb-1">
                                زمان خواب
                            </label>
                            <input
                                id="checkin-bedtime"
                                ...
                                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-left focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-700"
                            />
                        </div>
                        <div>
                            <label htmlFor="checkin-waketime" className="block text-xs text-slate-400 mb-1">
                                زمان بیداری
                            </label>
                            <input
                                id="checkin-waketime"
