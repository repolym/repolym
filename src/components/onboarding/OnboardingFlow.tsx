...
const OnboardingCardComponent: React.FC<{
                ...
                return (
                    <div className="w-full max-w-xs mx-auto aspect-square bg-gradient-to-br from-slate-100 to-indigo-100 rounded-3xl flex items-center justify-center relative overflow-hidden">
                        <div className="absolute inset-0 opacity-10">
                            <div className="absolute top-1/4 left-1/4 w-16 h-16 rounded-full bg-indigo-600 blur-2xl" />
                            <div className="absolute bottom-1/4 right-1/4 w-24 h-24 rounded-full bg-purple-600 blur-2xl" />
                        </div>
                        <div className="relative flex flex-col items-center">
                            <div className="w-20 h-20 rounded-2xl bg-indigo-600/20 flex items-center justify-center border-2 border-indigo-600/30">
                            ...
                            <div className="mt-4 flex gap-1">
                                {[1, 2, 3, 4, 5].map(i => (
                                    <div key={i} className={`w-1.5 h-8 rounded-full ${i <= 3 ? 'bg-indigo-600' : 'bg-gray-300'}`} />
                                ))}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="w-full max-w-xs mx-auto aspect-square bg-gradient-to-br from-purple-100 to-pink-100 rounded-3xl flex items-center justify-center relative">
                        <div className="flex items-end gap-2 h-32">
                            {[40, 65, 45, 80, 55, 70, 90].map((h, i) => (
                                <div key={i} className="w-6 rounded-t-lg bg-indigo-600/30" style={{ height: `${h}%` }} />
                            ))}
                        </div>
                    </div>
                )
            case 'privacy':
                return (
                    <div className="w-full max-w-xs mx-auto aspect-square bg-gradient-to-br from-blue-100 to-indigo-100 rounded-3xl flex items-center justify-center relative">
                        <div className="relative">
                            <div className="w-32 h-32 rounded-full bg-indigo-600/10 border-2 border-indigo-600/20 flex items-center justify-center">
            ...
            case 'survey':
                return (
                    <div className="w-full max-w-xs mx-auto aspect-square bg-gradient-to-br from-amber-100 to-orange-100 rounded-3xl flex items-center justify-center relative">
                        <div className="relative">
                            <div className="w-32 h-32 rounded-2xl bg-white/60 border-2 border-amber-600/20 flex items-center justify-center">
    ...
    return (
        ...
        >
            ...
            <h2 className="text-2xl md:text-3xl font-bold text-gray-800 mb-3 leading-tight">{card.title}</h2>
            <p className="text-sm md:text-base text-gray-500 max-w-sm leading-relaxed">{card.description}</p>
            <div className="mt-8 flex items-center gap-2">
                {Array.from({ length: total }).map((_, i) => (
                    <div
                        key={i}
                        className={`h-1.5 rounded-full transition-all duration-300 ${i <= progress ? 'w-6 bg-indigo-600' : 'w-2 bg-gray-200'}`}
                    />
                ))}
            </div>
            <div className="mt-8 flex items-center justify-between w-full gap-3">
                ...
                <div className="flex-1 flex justify-end">
                    <button
                        onClick={onNext}
                        className="inline-flex items-center gap-2 px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-xl transition-all shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40"
                    >
...
const OnboardingFlow: React.FC = () => {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-indigo-50 via-white to-purple-50">
            <div className="w-full max-w-2xl bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/50 p-6 md:p-10">
