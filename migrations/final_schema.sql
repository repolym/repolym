-- ============================================================
-- FINAL SCHEMA FOR REPOLYM
-- This script is idempotent and can be run on a fresh database
-- or on an existing one. It creates tables, indexes, functions,
-- triggers, RLS policies, and storage bucket.
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_users_olympiad_id ON users(olympiad_id);
...
-- Handle new user (insert into users and streaks)
...
-- get_analytics (merged performance + analytics)
...
BEGIN
    ...
    SELECT jsonb_build_object(
        'productivity_score', COALESCE(
            ...
            '{"productivity_score":0,"components":{"consistency":0,"goal_completion":0,"test_performance":0,"effective_study_time":0}}'::jsonb
        ),
        'recovery_score', COALESCE(
            ...
            '{"recovery_score":0,"components":{"sleep_consistency":0,"gap_recovery":0,"phone_usage_impact":0}}'::jsonb
        ),
        'study_streak', COALESCE(
            (SELECT jsonb_build_object(
            ...
            '{"current_streak":0,"longest_streak":0,"last_study_date":null}'::jsonb
        ),
        'study_consistency', COALESCE(
            (SELECT jsonb_build_object(
            ...
            '{"consistency_score":0,"active_days":0,"total_days":30,"target_active_days":20}'::jsonb
        ),
        'study_trend', COALESCE(
            (SELECT jsonb_build_object(
                'direction', 'stable',
                'slope', 0,
            ...
            '{"direction":"stable","slope":0,"average_change_per_day":0,"period_days":30}'::jsonb
        ),
        'moving_average', COALESCE(
            (SELECT jsonb_agg(jsonb_build_object('date', date, 'minutes', minutes, 'moving_avg_7d', moving_avg_7d)) FROM moving_avg_data),
            '[]'::jsonb
        ),
        'best_worst_day', COALESCE(
            (SELECT jsonb_build_object(
            ...
            '{"best_date":null,"best_date_minutes":null,"worst_date":null,"worst_date_minutes":null,"best_weekday_iso":null,"weekday_averages":null}'::jsonb
        ),
        'subject_distribution', COALESCE(
            (SELECT jsonb_agg(jsonb_build_object('subject_id', subject_id, 'subject_name', name, 'color', color, 'minutes', minutes, 'percent', percent)) FROM subject_dist_percent),
            '[]'::jsonb
        ),
        'sleep_statistics', COALESCE(
            (SELECT jsonb_build_object(
            ...
            '{"avg_sleep_hours":null,"min_sleep_hours":null,"max_sleep_hours":null,"logged_days":0}'::jsonb
        ),
        'phone_usage_statistics', COALESCE(
            (SELECT jsonb_build_object(
            ...
            '{"avg_phone_minutes":null,"min_phone_minutes":null,"max_phone_minutes":null,"logged_days":0}'::jsonb
        ),
        'personal_baseline', COALESCE(
            (SELECT jsonb_build_object(
            ...
            '{"baseline_avg_minutes":0,"baseline_avg_minutes_active_days":0,"baseline_days":0}'::jsonb
        ),
        'progress_trend', COALESCE(
            (SELECT jsonb_build_object(
            ...
            '{"direction":"insufficient_data","percent_change_vs_baseline":null,"current_avg_minutes":null}'::jsonb
        ),
        'test_stats', COALESCE(
            (SELECT jsonb_build_object(
            ...
            '{"total_tests":0,"accuracy_percent":0,"correct_percent":0,"wrong_percent":0,"skipped_percent":0,"avg_time_seconds":null,"trend":"insufficient_data"}'::jsonb
        ),
...
GRANT EXECUTE ON FUNCTION public.get_olympiad_leaderboard(text, date, integer, text, text) TO authenticated;

-- get_public_study_profile
...
GRANT EXECUTE ON FUNCTION public.get_public_study_profile TO authenticated;
