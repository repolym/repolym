...
-- ========================================
-- Migration 002: Performance dashboard extensions (tests columns + get_analytics)
-- ========================================
-- =============================================
-- Migration 002: Performance + Analytics merge
-- - Extends `tests` with optional per-question stats
--   (correct / wrong / skipped / total_questions / avg_time_seconds)
--   so the merged dashboard can show real solved-test KPIs
--   without breaking existing rows (all columns nullable).
-- - Extends the `get_analytics` RPC (same name & signature,
--   fully backward compatible) with:
--     test_stats                 -> solved/correct/wrong/skipped %, avg time, trend
--     subject_test_stats         -> per-subject accuracy (radar / topic mastery / weakness)
--     difficulty_distribution    -> distribution of study_sessions.question_difficulty
--     weekly_trend / monthly_trend -> zero-filled trend buckets for charts
-- Safe to run multiple times.
-- =============================================

-- 1. Extend tests table -------------------------------------------------
ALTER TABLE tests
    ADD COLUMN IF NOT EXISTS correct_count INTEGER CHECK (correct_count IS NULL OR correct_count >= 0),
    ADD COLUMN IF NOT EXISTS wrong_count INTEGER CHECK (wrong_count IS NULL OR wrong_count >= 0),
    ADD COLUMN IF NOT EXISTS skipped_count INTEGER CHECK (skipped_count IS NULL OR skipped_count >= 0),
    ADD COLUMN IF NOT EXISTS total_questions INTEGER CHECK (total_questions IS NULL OR total_questions >= 0),
    ADD COLUMN IF NOT EXISTS avg_time_seconds NUMERIC CHECK (avg_time_seconds IS NULL OR avg_time_seconds >= 0);

COMMENT ON COLUMN tests.correct_count IS 'Number of correctly answered questions (optional, per-question tracking)';
COMMENT ON COLUMN tests.wrong_count IS 'Number of incorrectly answered questions (optional)';
COMMENT ON COLUMN tests.skipped_count IS 'Number of skipped questions (optional)';
COMMENT ON COLUMN tests.total_questions IS 'Total number of questions in the test (optional)';
COMMENT ON COLUMN tests.avg_time_seconds IS 'Average time spent per question, in seconds (optional)';

-- 2. Extend get_analytics -------------------------------------------------
...
-- =============================================
-- Migration 002: Performance + Analytics merge
-- Run this against your existing Supabase project (SQL editor or `supabase db push`).
-- - Extends `tests` with optional per-question stats
--   (correct / wrong / skipped / total_questions / avg_time_seconds)
--   so the merged dashboard can show real solved-test KPIs
--   without breaking existing rows (all columns nullable).
