import{c as F,r as l,O as R,d as W,V as Te,W as Ee,j as e,_ as xe,$ as ue,a0 as he,a1 as te,a2 as B,a3 as Ae,a4 as ze,a5 as qe,a6 as Re,u as Le,a7 as Oe,a8 as G,a9 as De,aa as pe,ab as Je,ac as Ze,ad as Pe,ae as He,af as We,T as Fe,Z as Ue,ag as Ie}from"./index-DpGhi0uI.js";/**
 ...
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */const tt=[["path",{d:"M10 5H3",key:"1qgfaw"}],["path",{d:"M12 19H3",key:"yhmn1j"}],["path",{d:"M14 3v4",key:"1sua03"}],["path",{d:"M16 17v4",key:"1q0r14"}],["path",{d:"M21 12h-9",key:"1o4lsq"}],["path",{d:"M21 19h-5",key:"1rlt1p"}],["path",{d:"M21 5h-7",key:"1oszz2"}],["path",{d:"M8 10v4",key:"tgpxqk"}],["path",{d:"M8 12H3",key:"a7s4jb"}]],st=F("sliders-horizontal",tt),rt=s=>{const[r,o]=l.useState([]),[c,i]=l.useState(!1),[v,y]=l.useState(null),S=l.useRef(!0),x=l.useCallback(async(p=!1)=>{if(s&&!(c&&!p)){i(!0),y(null);try{const{data:d,error:m}=await R.from("chat_sessions").select("*").eq("user_id",s).is("deleted_at",null).order("updated_at",{ascending:!1});if(m)throw m;S.current&&o(d||[])}catch(d){const m=W(d);y(m)}finally{S.current&&i(!1)}}},[s]);l.useEffect(()=>(S.current=!0,x(),()=>{S.current=!1}),[x]);const a=l.useCallback(async(p={})=>{if(!s)return null;try{const d={user_id:s,title:p.title||null,messages:p.messages||[]},{data:m,error:w}=await R.from("chat_sessions").insert(d).select().single();if(w)throw w;return await x(!0),m}catch(d){return y(W(d)),null}},[s,x]),N=l.useCallback(async(p,d)=>{if(!s)return null;try{const m={...d,updated_at:new Date().toISOString()},{data:w,error:_}=await R.from("chat_sessions").update(m).eq("id",p).eq("user_id",s).select().single();if(_)throw _;return await x(!0),w}catch(m){return y(W(m)),null}},[s,x]),L=l.useCallback(async p=>{if(!s)return!1;try{const{error:d}=await R.from("chat_sessions").delete().eq("id",p).eq("user_id",s);if(d)throw d;return await x(!0),!0}catch(d){return y(W(d)),!1}},[s,x]),$=l.useCallback(async(p,d)=>{if(!s)return null;try{const{data:m,error:w}=await R.from("chat_sessions").select("messages").eq("id",p).single();if(w)throw w;const J=[...(m==null?void 0:m.messages)||[],...d];return await N(p,{messages:J})}catch(m){return y(W(m)),null}},[s,N]);return{sessions:r,loading:c,error:v,refetch:x,createSession:a,updateSession:N,deleteSession:L,appendMessages:$}},at=["message","summary","content","text","response","answer"],fe=["recommendations","strengths","weaknesses"],nt=s=>{const r=s.trim(),o=/^```(?:json|javascript|js)?\s*([\s\S]*?)\s*```$/i.exec(r);return o?o[1].trim():r},be=s=>{const r=s.trim();return r.startsWith("{")&&r.endsWith("}")||r.startsWith("[")&&r.endsWith("]")},ge=s=>{if(typeof s=="string")return s;if(s==null)return"";if(Array.isArray(s))return s.map(r=>`• ${typeof r=="string"?r:JSON.stringify(r)}`).join(`
`);if(typeof s=="object"){const r=s;for(const o of at){const c=r[o];if(typeof c=="string"&&c.trim()){let i=c.trim();if(be(i))try{return ge(JSON.parse(i))}catch{return i}const v=[];for(const y of fe){const S=r[y];if(Array.isArray(S)&&S.length>0){const x=y==="strengths"?"نقاط قوت":y==="weaknesses"?"نقاط ضعف":"پیشنهادات";v.push(`
...
`)),(g=h.recommendations)!=null&&g.length&&(b+=`
